...
<div class="card mb-4">
    ...
    <div class="card-body">
        ...
        <form action="{{ route('admin.invoice-templates.update', $invoiceTemplate->id) }}" method="POST" enctype="multipart/form-data">
            @csrf
            @method('PUT')
                ...
                <div class="col-lg-8">
                    <div class="row g-3 mb-3">
                        <div class="col-md-8">
                            ...
                            <input type="text" name="title" class="form-control @error('title') is-invalid @enderror" value="{{ old('title', $invoiceTemplate->title) }}" required>
                        </div>
                        <div class="col-md-4">
                            ...
                            <select name="type" id="type_select" class="form-select @error('type') is-invalid @enderror" onchange="toggleCustomType(this.value)" required>
                                ...
                                @if(isset($types))
                                    @foreach($types as $typeOpt)
                                        <option value="{{ $typeOpt }}" {{ old('type', $invoiceTemplate->type) == $typeOpt ? 'selected' : '' }}>{{ $typeOpt }}</option>
                                    @endforeach
                                @endif
                    ...
                    <div class="mb-3">
                        ...
                        <input type="text" name="subject" class="form-control" value="{{ old('subject', $invoiceTemplate->subject) }}">
                    </div>
                    ...
                    <div class="row g-3 mb-3">
                        <div class="col-md-6">
                            ...
                            @if($invoiceTemplate->header_image)
                                <div class="mt-2">
                                    <small class="text-success d-block mb-1"><i class="fas fa-check-circle"></i> Current Header:</small>
                                    <img src="{{ asset('storage/' . $invoiceTemplate->header_image) }}" alt="Header" class="img-thumbnail" style="max-height: 50px;">
                                </div>
                            @endif
                        </div>
                        <div class="col-md-6">
                            ...
                            @if($invoiceTemplate->footer_image)
                                <div class="mt-2">
                                    <small class="text-success d-block mb-1"><i class="fas fa-check-circle"></i> Current Footer:</small>
                                    <img src="{{ asset('storage/' . $invoiceTemplate->footer_image) }}" alt="Footer" class="img-thumbnail" style="max-height: 50px;">
                                </div>
                            @endif
                        </div>
                    </div>
                    ...
                    <div class="mb-3">
                        ...
                        <textarea name="content_body" id="editor" class="form-control @error('content_body') is-invalid @enderror" rows="12" required>{{ old('content_body', $invoiceTemplate->content_body) }}</textarea>
                    </div>

                    <div class="form-check form-switch mb-3">
                        <input class="form-check-input" type="checkbox" name="status" id="templateStatus" value="1" {{ old('status', $invoiceTemplate->status) ? 'checked' : '' }}>
