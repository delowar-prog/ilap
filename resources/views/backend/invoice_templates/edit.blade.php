                        ...
                        <select name="letter_head_id" id="letter_head_select" class="form-select @error('letter_head_id') is-invalid @enderror">
                            ...
                            @if(isset($letterHeads))
                                @foreach($letterHeads as $lh)
                                    <option value="{{ $lh->id }}" data-type="{{ $lh->type ?? 'all' }}" data-image="{{ asset('storage/' . $lh->image_path) }}" data-margin-top="{{ $lh->margin_top ?? 130 }}" data-margin-bottom="{{ $lh->margin_bottom ?? 120 }}" data-margin-left="{{ $lh->margin_left ?? 0 }}" data-margin-right="{{ $lh->margin_right ?? 0 }}" {{ (old('letter_head_id', $invoiceTemplate->letter_head_id) == $lh->id) ? 'selected' : '' }}>
