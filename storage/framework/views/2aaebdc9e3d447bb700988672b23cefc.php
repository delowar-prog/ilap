            ...
            <div class="profile-header">
                ...
                <div class="profile-avatar-container">
                    ...
                    <div class="profile-header-info">
                        <h2><?php echo e(ucwords(trim(($student->first_name ?? $preAssessment->first_name ?? '') . ' ' . ($student->middle_name ?? $preAssessment->middle_name ?? '') . ' ' . ($student->surname ?? $preAssessment->surname ?? '')))); ?></h2>
                ...
                <div class="tab-pane fade show active" id="tab-profile" role="tabpanel" aria-labelledby="tab-profile-btn">
                    ...
                    <div class="info-section">
                        ...
                        <div class="info-grid">
                            ...
                            <div class="info-item"><label>Title</label><span><?php echo e($student->title ?? $preAssessment->title ?? 'N/A'); ?></span></div>
                            ...
                            <div class="info-item">
                                ...
                                <span>
                                    <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(!empty($student->has_whatsapp) || !empty($preAssessment->has_whatsapp ?? false)): ?>
...
<div class="modal fade" id="generateLetterModal" tabindex="-1" aria-labelledby="generateLetterModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <form action="<?php echo e(route('admin.students.letters.generate', $student->id)); ?>" method="POST" enctype="multipart/form-data">
        ...
        <input type="hidden" name="is_customized" id="modal_is_customized" value="0">
             ...
             <div id="block_template">
                 ...
                 <div id="preview_container" class="d-none">
                     <div class="d-flex justify-content-between align-items-center mb-2">
                         ...
                         <span class="badge bg-success bg-opacity-10 text-success small"><i class="fas fa-magic me-1"></i> Auto-filled with student data</span>
                     </div>
                     <div class="border rounded-3 shadow-sm bg-secondary bg-opacity-10 p-3 d-flex flex-column align-items-center" style="max-height: 650px; overflow-y: auto;">
                         <div id="modal_preview_render_area" class="w-100 d-flex flex-column align-items-center"></div>
                         <textarea name="custom_content" id="modal_custom_content" class="d-none"></textarea>
                     </div>
                 </div>
             </div>
...
<div class="modal fade" id="generateInvoiceModal" tabindex="-1" aria-labelledby="generateInvoiceModalLabel" aria-hidden="true">
  <div class="modal-dialog modal-lg modal-dialog-centered">
    <form action="<?php echo e(route('admin.students.invoices.generate', $student->id)); ?>" method="POST" enctype="multipart/form-data" id="generateInvoiceForm">
        ...
        <input type="hidden" name="is_customized" id="modal_inv_is_customized" value="0">
        ...
        <div class="modal-content border-0 shadow-lg">
          ...
          <div class="modal-body p-4">
             ...
             <div id="inv_preview_container" class="d-none mb-3">
                 <div class="d-flex justify-content-between align-items-center mb-2">
                      <label class="form-label fw-bold text-success mb-0"><i class="fas fa-eye me-1"></i> Step 2: Live Invoice Preview (Editable before generating):</label>
                      <span class="badge bg-success bg-opacity-10 text-success px-2 py-1"><i class="fas fa-magic me-1"></i> Auto-populated</span>
                 </div>
                 <div class="border rounded-3 shadow-sm bg-secondary bg-opacity-10 p-3 d-flex flex-column align-items-center" style="max-height: 650px; overflow-y: auto;">
                      <div id="modal_inv_preview_render_area" class="w-100 d-flex flex-column align-items-center"></div>
                      <textarea name="custom_content" id="modal_invoice_custom_content" class="d-none"></textarea>
                 </div>
             </div>
...
$(document).ready(function() {
    $('#generateLetterModal').on('shown.bs.modal', function () {
        const val = $('#modal_template_id').val();
        if (val) {
            fetchTemplatePreview(val);
        }
    });

    $('#generateInvoiceModal').on('shown.bs.modal', function () {
        const val = $('#modal_invoice_template_id').val();
        if (val) {
            fetchInvoiceTemplatePreview(val);
        }
    });

    $('#generateLetterModal form, #generateInvoiceModal form').on('submit', function() {
        const $form = $(this);
        const $renderArea = $form.find('#modal_preview_render_area, #modal_inv_preview_render_area');
        if ($renderArea.length && $renderArea.find('.content-preview').length) {
            let fullHtml = '';
            $renderArea.find('.content-preview').each(function(i) {
                if (i > 0) fullHtml += '<div style="page-break-before: always;"></div>';
                fullHtml += $(this).html();
            });
            $form.find('textarea[name="custom_content"]').val(fullHtml);
        }
    });
});
...
function renderModalLivePreview($renderArea, data, inputFieldId) {
    ...
    if ($temp.children('.page-sheet').length > 0) {
        ...
        $temp.children('.page-sheet').each(function() {
            unwrappedHtml += $(this).html();
        });
        rawContent = unwrappedHtml;
    } else {
        rawContent = $temp.html();
    }

    const pageBreakPattern = /<div[^>]*style=["\'][^"\']*page-break-before\s*:\s*always[^"\']*["\'][^>]*>\s*<\/div>/i;
    let pages = rawContent.split(pageBreakPattern).filter(p => p.trim() !== '');
    if (pages.length === 0) {
        pages = [rawContent];
    }
    ...
    fetch(url)
        ...
        .then(data => {
            if (data.success) {
                ...
                renderModalLivePreview($('#modal_preview_render_area'), data, 'modal_custom_content');
            }
        })
    ...
    fetch(url)
        ...
        .then(data => {
            if (data.success) {
                ...
                renderModalLivePreview($('#modal_inv_preview_render_area'), data, 'modal_invoice_custom_content');
            }
        })
...
function initTableSearchAndPagination(inputId, tableId, paginationId, infoId, perPageSelectId = null, defaultItemsPerPage = 10) {
    ...
    // Add sortable styles and icons to column headers
    $table.find('thead th').each(function(colIdx) {
        const text = $(this).text().trim().toLowerCase();
        if (text === 'actions' || text === 'action') return;

        $(this).css({ 'cursor': 'pointer', 'user-select': 'none' }).attr('data-col', colIdx);
        if (!$(this).find('.sort-icon').length) {
            const iconClass = (colIdx === 0) ? 'fa-sort-up text-primary' : 'fa-sort text-muted';
            $(this).append(` <i class="fas ${iconClass} sort-icon ms-1" style="font-size: 0.75rem;"></i>`);
        }
    });
    ...
    function sortRows() {
        ...
        rows.sort(function(a, b) {
            let cellA = $(a).children('td').eq(sortColIdx).text().trim();
            let cellB = $(b).children('td').eq(sortColIdx).text().trim();

            const numA = parseFloat(cellA.replace(/[^0-9.-]+/g, ''));
            const numB = parseFloat(cellB.replace(/[^0-9.-]+/g, ''));

            if (!isNaN(numA) && !isNaN(numB) && cellA.match(/^[£$€\d.,\s#]+$/) && cellB.match(/^[£$€\d.,\s#]+$/)) {
    ...
    $table.find('thead th[data-col]').on('click', function() {
        const clickedCol = parseInt($(this).attr('data-col'));
        if (sortColIdx === clickedCol) {
            sortDir = (sortDir === 'asc') ? 'desc' : 'asc';
        } else {
            sortColIdx = clickedCol;
            sortDir = 'asc';
        }

        $table.find('thead th .sort-icon').removeClass('fa-sort-up fa-sort-down fa-sort text-primary').addClass('fa-sort text-muted');
        const $icon = $(this).find('.sort-icon');
        if (sortDir === 'asc') {
            $icon.removeClass('fa-sort text-muted').addClass('fa-sort-up text-primary');
        } else {
            $icon.removeClass('fa-sort text-muted').addClass('fa-sort-down text-primary');
        }
